...
1. **初回(generate-tree)**: 目標から逆算した木を4〜6本と、各木の枝をAIが生成する。
   マクロ層(木)は目標逆算、ミクロ層(枝)はベースライン骨格に準拠。
   **葉はこの時点では生成しない**
2. **枝クリック時(get-or-generate-leaves)**: その枝の葉(約1週間分の毎日タスク)を生成。
...
- **保存方針**: 木と枝は `trees.tree_data`(jsonb)に丸ごと保存(MVPでは正規化しない)。
  **葉と日次記録は更新頻度が高いため `leaves` / `daily_logs` の独立テーブル**に置く
...
- ツリーは中央幹レイアウト: 下部のSTART円から縦の幹が伸び、マイルストーンのピルが幹上に並び、
  枝(スキルカード)は左右交互に分岐、関連技術はさらに外側へ小ピルでファン状に枝分かれする。
  マイルストーンごとにパステルのアクセントカラーを循環使用(詳細は docs/design.md)
