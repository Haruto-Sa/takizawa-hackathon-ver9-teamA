...
export function Onboarding({ onComplete }: { onComplete: (id: string, tree: SkillTree) => void }) {
  ...
  return <main className="onboarding"><header className="brand"><span className="brand-mark">↗</span> SKILL PATH</header><section className="onboarding-card"><div className="progress"><span>STEP {step} / 4</span><div>{[1,2,3,4].map((n)=><i key={n} className={n<=step?'active':''}/>)}</div></div>
